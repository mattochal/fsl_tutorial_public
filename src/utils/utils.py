...
from datasets.fish import get_FishRecognition
...
def get_data(args):
    ...
    if (args.conventional_split is None and args.model in ["baseline"]):
...
def get_backbone(args, device):
    ...
    if args.model in ["maml", "protomaml"]:
...
def get_model(backbone, tasks, datasets, args, device):
    """
    ...
    if args.model in ["baseline", "maml", "protomaml"]:
        output_dims = dict()
    ...
    model = MODELS[args.model](backbone, args.model_args, device)
    model.setup_model()
